...
def _segment_preview(segment_items: list[dict]) -> dict:
    ...
    return {
        ...
        "sample_sheet_name": metadata.get("sheet_name", "n/a"),
...
def run_example(client=None):
    ...
    artifacts = {"example_id": "08-excel-csv-basic", "title": "Excel CSV workflow", "status": "ok"}
    section = 1
    try:
        print_section(section, "Create project")
        project = api.create_project(project_name("08-excel-csv-basic"), description=artifacts["title"])
        ...
        source_path = docs_path(EXCEL_FILE)
        upload = api.upload_document(project_id, source_path, EXCEL_MIME)
        ...
        print_section(section, "Load documents (excel loader -> csv)")
        loaded = api.load_documents(document_version_id, loader_type="excel", loader_params={"output_format": "csv"})
        ...
        print_section(section, "Create segments (csv_table)")
        ...
        idx = api.create_index(project_id, "08_excel_csv_basic", provider="chroma", config=INDEX_CONFIG)
