...
MARKDOWN_FILE = "07_md_table_summary_input_ru.md"
...
MARKDOWN_TABLE_SPLITTER_PARAMS = {
...
def _segment_summary_preview(segment_items: list[dict]) -> dict:
    ...
    items_with_summary_prefix = [item for item in segment_items if (item.get("content") or "").lstrip().startswith("Table Summary:")]
    sample = items_with_summary_prefix[0] if items_with_summary_prefix else segment_items[0]
    sample_meta = sample.get("metadata") or {}
    sample_content = sample.get("content") or ""

    return {
        "items": len(segment_items),
        "items_with_summary_prefix": len(items_with_summary_prefix),
        "summary_prefix_in_sample": sample_content.lstrip().startswith("Table Summary:"),
        "table_summary_metadata_in_sample": "table_summary" in sample_meta,
        "content_preview": sample_content[:120],
    }


def run_example(client=None):
    api = client or default_client()
    artifacts = {"example_id": "07-md-table-summary", "title": "Markdown table summary workflow", "status": "ok"}
    section = 1
    try:
        print_section(section, "Create project")
        project = api.create_project(project_name("07-md-table-summary"), description=artifacts["title"])
        ...
        source_path = docs_path(MARKDOWN_FILE)
        upload = api.upload_document(project_id, source_path, MARKDOWN_MIME)
        ...
        print_section(section, "Create segments (markdown_table + summaries)")
        seg = api.create_segments(
            document_set_version_id,
            split_strategy="markdown_table",
            splitter_params=MARKDOWN_TABLE_SPLITTER_PARAMS,
        )
        ...
        idx = api.create_index(project_id, "07_md_table_summary", provider="chroma", config=INDEX_CONFIG)
