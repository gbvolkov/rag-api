...
def run_example(client=None):
    ...
    try:
        ...
        upload = api.upload_document(artifacts["project_id"], docs_path("anon.log"), "text/plain")
        ...
        print_section(section, "Create segments (regex loader)")
        seg = api.create_segments(
            artifacts["document_version_id"],
            loader_type="regex",
            loader_params={"patterns": PATTERNS},
        )
        ...
        print_section(section, "Create chunks (regex_hierarchy)")
        chunk = api.create_chunks(
            artifacts["segment_set_version_id"],
            strategy="regex_hierarchy",
            chunker_params={"patterns": PATTERNS},
        )
