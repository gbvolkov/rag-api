...
REGEX_LOADER_PARAMS = {"patterns": PATTERNS}
REGEX_SPLITTER_PARAMS = {"patterns": PATTERNS}
...
def _segment_preview(segment_items: list[dict]) -> dict:
    ...
    return {
        ...
        "sample_title": metadata.get("title", "n/a"),
...
def run_example(client=None):
    ...
    artifacts = {"example_id": "11-log-regex-loader", "title": "Log regex loader workflow", "status": "ok"}
    section = 1
    try:
        print_section(section, "Create project")
        project = api.create_project(project_name("11-log-regex-loader"), description=artifacts["title"])
        ...
        source_path = docs_path(LOG_FILE)
        upload = api.upload_document(project_id, source_path, LOG_MIME)
        ...
        print_section(section, "Load documents (regex loader)")
        loaded = api.load_documents(document_version_id, loader_type="regex", loader_params=REGEX_LOADER_PARAMS)
        ...
        print_section(section, "Create segments (regex_hierarchy)")
        seg = api.create_segments(
            document_set_version_id,
            split_strategy="regex_hierarchy",
            splitter_params=REGEX_SPLITTER_PARAMS,
        )
        ...
        idx = api.create_index(project_id, "11_log_regex_loader", provider="chroma", config=INDEX_CONFIG)
