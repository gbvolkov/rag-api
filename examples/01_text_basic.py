...
def run_example(client=None):
    ...
    artifacts = {"example_id": "01-text-basic", "title": "Basic text workflow", "status": "ok"}
    section = 1
    try:
        print_section(section, "Create project")
        project = api.create_project(project_name("01-text-basic"), description=artifacts["title"])
        ...
        print_section(section, "Create chunks (token)")
        chunk = api.split_segment_set(
            artifacts["segment_set_version_id"],
            strategy="token",
            splitter_params={"chunk_size": 150, "chunk_overlap": 15},
        )
